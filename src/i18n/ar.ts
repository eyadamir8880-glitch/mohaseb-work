const ar = {
  app: {
    name: 'محاسبة يد',
    tagline: 'نظام محاسبة وإدارة موارد',
    loading: 'جار التحميل...',
    error: 'حدث خطأ',
    retry: 'إعادة المحاولة',
    save: 'حفظ',
    cancel: 'إلغاء',
    delete: 'حذف',
    edit: 'تعديل',
    create: 'إنشاء',
    search: 'بحث...',
    noResults: 'لا توجد نتائج',
    confirmDelete: 'هل أنت متأكد من الحذف؟',
    yesDelete: 'نعم، احذف',
    actions: 'إجراءات',
    status: 'الحالة',
    date: 'التاريخ',
    amount: 'المبلغ',
    total: 'الإجمالي',
    back: 'رجوع',
    next: 'التالي',
    previous: 'السابق',
    export: 'تصدير',
    import: 'استيراد',
    print: 'طباعة',
    download: 'تحميل',
    upload: 'رفع',
    filter: 'تصفية',
    clear: 'مسح',
    all: 'الكل',
    from: 'من',
    to: 'إلى',
    apply: 'تطبيق',
    reset: 'إعادة تعيين',
    close: 'إغلاق',
    toggleSidebar: 'فتح/إغلاق القائمة',
    confirm: 'تأكيد',
    submit: 'إرسال',
    add: 'إضافة',
    remove: 'إزالة',
    update: 'تحديث',
    view: 'عرض',
    duplicate: 'نسخ',
    email: 'بريد إلكتروني',
    phone: 'هاتف',
    address: 'عنوان',
    notes: 'ملاحظات',
    reference: 'مرجع',
    description: 'وصف',
    category: 'تصنيف',
    type: 'نوع',
    currency: 'عملة',
    language: 'اللغة',
    theme: 'المظهر',
    light: 'فاتح',
    dark: 'داكن',
    searchGlobal: 'ابحث عن أي شيء...',
    noData: 'لا توجد بيانات',
    overview: 'نظرة عامة',
    deleteAll: 'حذف الكل',
    deleteAllWarning: 'لا يمكن التراجع عن هذا الإجراء. هل أنت متأكد؟',
    select: 'اختر...',
    no: 'لا',
    deleteConfirm: 'هل أنت متأكد من حذف هذا العنصر؟',
    ok: 'حسناً',
    open: 'مفتوح',
    in: 'وارد',
    out: 'صادر',
    adj: 'تسوية',
    active: 'نشط',
    outOfStock: 'نفذ من المخزون',
    created: 'تم الإنشاء',
    updated: 'تم التحديث',
    deleted: 'تم الحذف',
    default: 'افتراضي',
  },

  nav: {
    dashboard: 'لوحة القيادة',
    invoices: 'الفواتير',
    customers: 'العملاء',
    products: 'المنتجات',
    warehouse: 'المستودع',
    treasury: 'الخزينة',
    categories: 'التصنيفات',
    reports: 'التقارير',
    settings: 'الإعدادات',
    logout: 'تسجيل الخروج',
    profile: 'الملف الشخصي',
  },

  dashboard: {
    title: 'لوحة القيادة',
    totalRevenue: 'إجمالي الإيرادات',
    totalExpenses: 'إجمالي المصروفات',
    netProfit: 'صافي الربح',
    outstandingInvoices: 'الفواتير المستحقة',
    currentBalance: 'الرصيد الحالي',
    revenueVsExpenses: 'الإيرادات مقابل المصروفات',
    salesByCategory: 'المبيعات حسب التصنيف',
    monthlyTrends: 'الاتجاهات الشهرية',
    cashFlow: 'التدفق النقدي',
    recentActivity: 'النشاط الأخير',
    quickActions: 'إجراءات سريعة',
    newInvoice: 'فاتورة جديدة',
    newCustomer: 'عميل جديد',
    newProduct: 'منتج جديد',
    newTransaction: 'معاملة جديدة',
    viewAll: 'عرض الكل',
  },

  invoices: {
    title: 'الفواتير',
    newInvoice: 'فاتورة جديدة',
    editInvoice: 'تعديل الفاتورة',
    invoiceNumber: 'رقم الفاتورة',
    customer: 'العميل',
    issueDate: 'تاريخ الإصدار',
    dueDate: 'تاريخ الاستحقاق',
    subtotal: 'المجموع الفرعي',
    discount: 'الخصم',
    tax: 'الضريبة',
    grandTotal: 'الإجمالي النهائي',
    items: 'البنود',
    product: 'المنتج',
    quantity: 'الكمية',
    price: 'السعر',
    total: 'الإجمالي',
    maxQty: 'الحد الأقصى',
    addItem: 'إضافة بند',
    removeItem: 'إزالة بند',
    statuses: { draft: 'مسودة', sent: 'مرسلة', paid: 'مدفوعة', partially_paid: 'مدفوعة جزئياً', overdue: 'متأخرة', cancelled: 'ملغاة', partially_returned: 'مرتجعة جزئياً', fully_returned: 'مرتجعة بالكامل' },
    markAsSent: 'تحديد كمرسلة',
    markAsPaid: 'تحديد كمدفوعة',
    markAsOverdue: 'تحديد كمتأخرة',
    cancelInvoice: 'إلغاء الفاتورة',
    sendEmail: 'إرسال بريد إلكتروني',
    printInvoice: 'طباعة الفاتورة',
    emailSent: 'تم إرسال الفاتورة بنجاح',
    invoiceFrom: 'الفاتورة من',
    invoiceTo: 'الفاتورة إلى',
    paymentTerms: 'شروط الدفع',
    thanks: 'شكراً لتعاملكم معنا!',
    addNew: 'فاتورة جديدة',
    date: 'التاريخ',
    unitPrice: 'سعر الوحدة',
    discountTotal: 'إجمالي الخصم',
    taxTotal: 'إجمالي الضريبة',
    notes: 'ملاحظات',
    reference: 'مرجع',
    paymentHistory: 'سجل المدفوعات',
    noPayments: 'لا توجد مدفوعات مسجلة',
    paid: 'مدفوعة',
    paidAmount: 'المبلغ المدفوع',
    remaining: 'المتبقي',
    partiallyPaid: 'مدفوعة جزئياً',
    paymentModal: {
      title: 'تسجيل دفعة',
      amount: 'المبلغ',
      date: 'تاريخ الدفع',
      method: 'طريقة الدفع',
      referenceNumber: 'رقم المرجع',
      confirm: 'تأكيد الدفع',
    },
  },

  customers: {
    title: 'العملاء',
    newCustomer: 'عميل جديد',
    editCustomer: 'تعديل العميل',
    addNew: 'عميل جديد',
    name: 'الاسم',
    taxNumber: 'الرقم الضريبي',
    creditLimit: 'الحد الائتماني',
    totalInvoices: 'إجمالي الفواتير',
    totalPaid: 'إجمالي المدفوع',
    totalDue: 'إجمالي المستحق',
    totalInvoiced: 'إجمالي الفواتير',
    transactionHistory: 'سجل المعاملات',
    statement: 'كشف حساب',
    statementReport: 'كشف حساب العميل',
    address: 'العنوان',
    email: 'بريد إلكتروني',
    phone: 'هاتف',
  },

  products: {
    title: 'المنتجات',
    newProduct: 'منتج جديد',
    editProduct: 'تعديل المنتج',
    name: 'اسم المنتج',
    sku: 'رمز المنتج',
    barcode: 'الباركود',
    description: 'الوصف',
    purchasePrice: 'سعر الشراء',
    sellingPrice: 'سعر البيع',
    profitMargin: 'هامش الربح',
    unit: 'الوحدة',
    category: 'التصنيف',
    stock: 'المخزون',
    trackInventory: 'تتبع المخزون',
    currentStock: 'المخزون الحالي',
    lowStockThreshold: 'حد المخزون المنخفض',
    lowStock: 'مخزون منخفض',
    inStock: 'متوفر',
    outOfStock: 'غير متوفر',
    bulkImport: 'استيراد مجمع',
    bulkExport: 'تصدير مجمع',
    downloadTemplate: 'تحميل القالب',
    importResults: 'نتائج الاستيراد',
    rowsProcessed: 'تمت معالجة {n} صف',
    errorsFound: 'تم العثور على {n} خطأ',
    duplicateSku: 'رمز منتج مكرر',
    missingRequired: 'حقول إجبارية ناقصة',
    invalidPrice: 'سعر غير صالح',
  },

  warehouse: {
    title: 'المستودع',
    stockLevels: 'مستويات المخزون',
    movements: 'الحركات',
    newMovement: 'حركة جديدة',
    movementType: 'نوع الحركة',
    in: 'وارد',
    out: 'صادر',
    adjustment: 'تسوية',
    reason: 'السبب',
    warehouseName: 'المستودع',
    inventoryValuation: 'تقييم المخزون',
    totalStockValue: 'القيمة الإجمالية للمخزون',
    lowStockAlerts: 'تنبيهات المخزون المنخفض',
    lowStock: 'مخزون منخفض',
    outOfStock: 'نفد من المخزون',
    stockMovements: 'حركات المخزون',
    totalProducts: 'إجمالي المنتجات',
    totalValue: 'القيمة الإجمالية',
  },

  returns: {
    title: 'المرتجعات',
    addNew: 'مرتجع جديد',
    customerReturn: 'مرتجع عميل',
    originalInvoice: 'الفاتورة الأصلية',
    reason: 'السبب',
    condition: 'الحالة',
    good: 'سليم',
    bad: 'تالف',
    refundAmount: 'مبلغ الاسترداد',
    returnNumber: 'رقم المرتجع',
    refundMethod: 'طريقة الاسترداد',
  },

  treasury: {
    title: 'الخزينة',
    overview: 'نظرة عامة',
    transactions: 'المعاملات',
    accounts: 'الحسابات',
    newTransaction: 'معاملة جديدة',
    addTransaction: 'إضافة معاملة',
    accountName: 'اسم الحساب',
    account: 'الحساب',
    balance: 'الرصيد',
    income: 'إيراد',
    expense: 'مصروف',
    paymentMethod: 'طريقة الدفع',
    paymentMethodBreakdown: 'تفصيل طرق الدفع',
    cash: 'نقداً',
    bank: 'بنك',
    card: 'بطاقة',
    check: 'شيك',
    reconciled: 'تمت التسوية',
    unreconciled: 'لم تتم التسوية',
    transfer: 'تحويل',
    newAccount: 'حساب جديد',
    manageCategories: 'إدارة التصنيفات',
    incomeCategories: 'تصنيفات الإيرادات',
    expenseCategories: 'تصنيفات المصروفات',
    recurringTransactions: 'معاملات متكررة',
    forecast: 'توقعات التدفق النقدي',
    accountStatement: 'كشف الحساب',
    reconciliation: 'تسوية الحسابات',
    description: 'الوصف',
    fromAccount: 'من حساب',
    toAccount: 'إلى حساب',
    total: 'الإجمالي',
    reconcile: 'تسوية',
    unreconcile: 'إلغاء التسوية',
    recurring: 'متكرر',
    everyDay: 'كل يوم',
    everyWeek: 'كل أسبوع',
    everyMonth: 'كل شهر',
    everyYear: 'كل سنة',
  },

  categories: {
    title: 'التصنيفات',
    newCategory: 'تصنيف جديد',
    editCategory: 'تعديل التصنيف',
    addNew: 'تصنيف جديد',
    name: 'اسم التصنيف (إنجليزي)',
    nameAr: 'اسم التصنيف (عربي)',
    parent: 'التصنيف الأب',
    type: 'نوع التصنيف',
    product: 'منتج',
    income: 'إيراد',
    expense: 'مصروف',
    none: 'بدون (مستوى رئيسي)',
  },

  fiscalYears: {
    title: 'السنوات المالية',
    create: 'إنشاء سنة مالية',
    edit: 'تعديل سنة مالية',
    name: 'الاسم',
    nameAr: 'الاسم (عربي)',
    startDate: 'تاريخ البداية',
    endDate: 'تاريخ النهاية',
    isClosed: 'مغلقة',
    close: 'إغلاق السنة المالية',
    confirmClose: 'هل أنت متأكد من إغلاق هذه السنة المالية؟',
    closedAt: 'أغلقت في',
    closeEntries: 'قيود الإغلاق',
    noData: 'لا توجد سنوات مالية',
  },

  reports: {
    title: 'التقارير',
    profitLoss: 'الأرباح والخسائر',
    profitAndLoss: 'الأرباح والخسائر',
    balanceSheet: 'الميزانية العمومية',
    cashFlow: 'التدفق النقدي',
    trialBalance: 'ميزان المراجعة',
    salesByProduct: 'المبيعات حسب المنتج',
    salesByCustomer: 'المبيعات حسب العميل',
    salesByPeriod: 'المبيعات حسب الفترة',
    purchaseReport: 'تقرير المشتريات',
    inventoryReport: 'تقرير المخزون',
    inventoryValuation: 'تقييم المخزون',
    treasuryBreakdown: 'تفصيل الخزينة',
    customerStatement: 'كشف حساب العميل',
    dateRange: 'نطاق التاريخ',
    financial: 'التقارير المالية',
    generateReport: 'إنشاء التقرير',
    totalRevenue: 'إجمالي الإيرادات',
    totalExpenses: 'إجمالي المصروفات',
    netProfit: 'صافي الربح',
    exportPdf: 'تصدير PDF',
    exportCsv: 'تصدير CSV',
    exportExcel: 'تصدير Excel',
    from: 'من',
    to: 'إلى',
    generalLedger: 'دفتر الأستاذ العام',
    agedReceivables: 'ذمم العملاء',
    agedPayables: 'ذمم الموردين',
    accountCode: 'رمز الحساب',
    accountName: 'اسم الحساب',
    debit: 'مدين',
    credit: 'دائن',
    balance: 'الرصيد',
    openingBalance: 'الرصيد الافتتاحي',
    closingBalance: 'الرصيد الختامي',
    period: 'الفترة',
    details: 'التفاصيل',
    summary: 'الملخص',
    totalDue: 'إجمالي المستحق',
    current: 'جاري',
    days31to60: '31-60 يوم',
    days61to90: '61-90 يوم',
    over90Days: 'أكثر من 90 يوم',
    operatingActivities: 'الأنشطة التشغيلية',
    investingActivities: 'الأنشطة الاستثمارية',
    financingActivities: 'الأنشطة التمويلية',
    netCashFlow: 'صافي التدفق النقدي',
    beginningCash: 'النقدية في بداية الفترة',
    endingCash: 'النقدية في نهاية الفترة',
    totalDebits: 'إجمالي المدين',
    totalCredits: 'إجمالي الدائن',
    accountType: 'نوع الحساب',
    allAccounts: 'جميع الحسابات',
    totalAssets: 'إجمالي الأصول',
    totalLiabilities: 'إجمالي الخصوم',
    totalEquity: 'إجمالي حقوق الملكية',
    income: 'الإيرادات',
    expenses: 'المصروفات',
    grossProfit: 'إجمالي الربح',
    netIncome: 'صافي الدخل',
    costOfSales: 'تكلفة المبيعات',
    operatingExpenses: 'المصروفات التشغيلية',
    otherIncome: 'إيرادات أخرى',
    otherExpenses: 'مصروفات أخرى',
    reference: 'المرجع',
    debitBalance: 'رصيد مدين',
    creditBalance: 'رصيد دائن',
    difference: 'الفرق',
    inBalance: 'متوازن',
    outOfBalance: 'غير متوازن',
  },

  settings: {
    title: 'الإعدادات',
    company: 'الشركة',
    companyName: 'اسم الشركة (إنجليزي)',
    companyNameAr: 'اسم الشركة (عربي)',
    logo: 'شعار الشركة',
    address: 'العنوان (إنجليزي)',
    addressAr: 'العنوان (عربي)',
    taxInfo: 'المعلومات الضريبية',
    currency: 'العملة',
    defaultCurrency: 'العملة الافتراضية',
    invoice: 'الفاتورة',
    invoicePrefix: 'بادئة الفاتورة',
    invoiceNextNumber: 'الرقم التالي للفاتورة',
    invoiceTerms: 'شروط الفاتورة (إنجليزي)',
    invoiceTermsAr: 'شروط الفاتورة (عربي)',
    tax: 'الضريبة',
    taxRate: 'نسبة الضريبة',
    paymentMethods: 'طرق الدفع',
    treasury: 'الخزينة',
    userManagement: 'إدارة المستخدمين',
    language: 'اللغة',
    theme: 'المظهر',
    saveSettings: 'حفظ الإعدادات',
    settingsSaved: 'تم حفظ الإعدادات بنجاح',
    dataManagement: 'إدارة البيانات',
    exportAll: 'تصدير جميع البيانات',
    importHistory: 'سجل الاستيراد',
    resetDemo: 'إعادة تعيين البيانات التجريبية',
    aiAssistant: 'المساعد الذكي',
    aiModel: 'نموذج الذكاء الاصطناعي',
    aiModelHint: 'اختر نموذج جيميني الذي يستخدمه المساعد الذكي. حدود الحزمة المجانية تختلف حسب النموذج — إذا وصلت للحد، جرّب Flash Lite.',
  },

  auditLog: {
    title: 'سجل التدقيق',
    action: 'الإجراء',
    module: 'الوحدة',
    user: 'المستخدم',
    timestamp: 'التاريخ والوقت',
    ip: 'عنوان IP',
    recordId: 'معرف السجل',
    oldValues: 'القيم القديمة',
    newValues: 'القيم الجديدة',
    details: 'التفاصيل',
    exportLog: 'تصدير السجل',
  },

  notifications: {
    title: 'الإشعارات',
  },

  common: {
    darkMode: 'الوضع الداكن',
    lightMode: 'الوضع الفاتح',
    loadSession: 'تحميل الجلسة',
    saveSession: 'حفظ الجلسة',
    invalidBackup: 'ملف النسخ الاحتياطي غير صالح',
  },

  customerAccount: {
    title: 'كشف حساب العميل',
    selectCustomer: 'اختر العميل',
    selectCustomerHint: 'اختر عميلاً لعرض كشف حسابه',
    from: 'من تاريخ',
    to: 'إلى تاريخ',
    openingBalance: 'الرصيد الافتتاحي',
    closingBalance: 'الرصيد الختامي',
    date: 'التاريخ',
    description: 'الوصف',
    descriptionAr: 'الوصف (عربي)',
    reference: 'المرجع',
    debit: 'مدين',
    credit: 'دائن',
    totalDebit: 'إجمالي المدين',
    totalPaid: 'إجمالي المدفوع',
    remaining: 'المتبقي',
    balance: 'الرصيد',
    type: 'النوع',
    payment: 'دفعة',
    addEntry: 'إضافة قيد',
    noTransactions: 'لا توجد معاملات',
    print: 'طباعة',
  },

  import: {
    title: 'استيراد المنتجات',
    dragDrop: 'اسحب وأفلت ملفاً، أو انقر للتصفح',
    selectFile: 'اختر ملف',
    selectAll: 'تحديد الكل',
    importSelected: 'استيراد المحدد',
    productName: 'اسم المنتج',
    productNumber: 'رقم المنتج',
    itemNumber: 'رقم البند',
    sellingPrice: 'سعر البيع',
    price: 'السعر',
    sku: 'رمز المنتج',
    serial: 'الرقم التسلسلي',
    duplicateSku: 'رمز منتج مكرر',
    duplicateSerial: 'رقم تسلسلي مكرر',
    missingSku: 'رمز المنتج ناقص',
    missingName: 'الاسم ناقص',
    missingPrice: 'السعر ناقص',
    expectedColumns: 'الأعمدة المتوقعة',
    errors: 'أخطاء',
    processing: 'جارٍ المعالجة...',
    valid: 'صالح',
    skipped: 'تم التجاهل',
    imported: 'تم الاستيراد',
    totalRows: 'إجمالي الصفوف',
    filename: 'اسم الملف',
    uploadedAt: 'تم الرفع في',
    importComplete: 'اكتمل الاستيراد',
    importAnother: 'استيراد آخر',
  },

  customerImport: {
    title: 'استيراد العملاء',
    customerName: 'اسم العميل',
    totalDebt: 'إجمالي الفواتير',
    collected: 'المحصل',
    remaining: 'المتبقي',
    missingName: 'اسم العميل ناقص',
    duplicateName: 'اسم عميل مكرر',
    exists: 'موجود - سيتم التحديث',
    treasuryNote: 'استيراد رصيد افتتاحي',
    openingBalance: 'رصيد افتتاحي',
    paymentReceived: 'استلام دفعة',
    updated: 'تم التحديث',
  },

  ai: {
    title: 'المساعد الذكي',
    subtitle: 'الفواتير ومساعدة الأعمال',
    greeting: 'أهلاً! اسألني عن أي شيء يخص نشاطك التجاري، أو صف لي فاتورة — وأستطيع قراءة الصور وملفات PDF وExcel أيضاً.',
    placeholder: 'اكتب رسالتك… أو أرفق ملفاً',
    attach: 'إرفاق ملف',
    clearHistory: 'مسح',
    invoiceTitle: 'مساعد الفواتير الذكي',
    invoiceSubtitle: 'صف فاتورة أو أرفق ملاحظة/صورة/PDF/Excel',
    suggestionDebt: 'كم مجموع ما يدين به عملائي؟',
    suggestionOverdue: 'أي الفواتير متأخرة؟',
    suggestionTop: 'ما هي أفضل المنتجات مبيعاً؟',
    suggestionToday: 'لخص مبيعات اليوم',
    suggestionInvoice: 'أنشئ فاتورة من صورة أو وصف',
    suggestionInvoiceExample: 'فاتورة لأحمد: 2 × كولا بسعر 10',
    draftPreview: 'مسودة الفاتورة',
    noCustomer: 'لا يوجد عميل محدد',
    newProductTag: 'جديد',
    newRecordsNote: 'سيتم إنشاء هذه عند التطبيق:',
    applyToForm: 'تطبيق على نموذج الفاتورة',
    applied: 'تم تطبيق المسودة',
    appliedDesc: 'راجع النموذج ثم اضغط حفظ للتأكيد.',
    applyNoCustomer: 'أضف أو أنشئ عميلاً قبل التطبيق.',
    applyMissingProduct: 'بعض الأصناف بدون منتج. راجع المسودة.',
  },

  404: {
    title: 'الصفحة غير موجودة',
    message: 'الصفحة التي تبحث عنها غير موجودة.',
    backToDashboard: 'العودة إلى لوحة القيادة',
  },
};

export default ar;
